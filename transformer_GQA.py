...
class MHA(nn.Module):
    def __init__(self,d_model,heads,Q_dim,dropout=None,GQA = None):
    ...
    def forward(self, embeddings, use_cache=False, past_kv=None):
        ...
        #GQA is like an downsampling and then storing for saving memory while getting similar or close reconstruction of the original kv_cache
        if GQA is not None:
            k = k[:, :, ::GQA, :]
            v = v[:, :, ::GQA, :]
        if use_cache:
        ...
        #reversing the effects of GQA by interpolation 
        expanded_k = torch.zeros(k.shape[0], k.shape[1], n, k.shape[3]).to(k.device)
        expanded_v = torch.zeros(v.shape[0], v.shape[1], n, v.shape[3]).to(v.device)
        expanded_k[:, :, ::GQA, :] = k
        expanded_v[:, :, ::GQA, :] = v
        expanded_k[:, :, 1::GQA, :] = expanded_k[:, :, ::GQA, :].repeat(1, 1, GQA-1, 1)
        expanded_v[:, :, 1::GQA, :] = expanded_v[:, :, ::GQA, :].repeat(1, 1, GQA-1, 1)
        # we pass expanded_k and expanded_v instead of k and v becaus e they are the reconstructions of real K and V from our downsampled K and V
        attn_output = self.attn(q, expanded_k, expanded_v,n)
...
class transformers(nn.Module):
    ...
    def forward(self, embeddings, use_cache=False, past_kv=None):
        #Pass KV cache parameters to MHA 
